...
//  Vec2f target = new Vec2f(100.0f, 100.0f);
 ...
 glfwSetMouseButtonCallback(window, mouseButtonCallback = GLFWMouseButtonCallback.create( (long windowId, int button, int action, int mods) ->
 {
    if (button == GLFW_MOUSE_BUTTON_1 && action == GLFW_PRESS)
			{ InverseKinematicsVisualization.leftMouseButtonDown = true;}
    else { InverseKinematicsVisualization.leftMouseButtonDown = false;}
 glfwSetCursorPosCallback(window, cursorPosCallback = GLFWCursorPosCallback.create( (long windowId2, double mouseX,  double mouseY) ->
 {
 ...
 mTargetPoint.draw(new Vec2f(5.0f,10.0f), Utils.YELLOW, 50.0f, mvpMatrix); // Solve the chain
 }
 }));
}));
 ...
 public static void main(String[] args) { new InverseKinematicsVisualization().run(); }
}